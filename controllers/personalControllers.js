                  ...
                  personalDAO.getgiftsweet(propsId, function(err, results3) {
                    if (err) {
                      ...
                      personalDAO.selectsweet(userId, function(err, results4) {
                        console.log(addId)
                        console.log(results4)
                        var sweetExist = false
                        for (var m = 0; m < results4.length; m++) {
                          console.log(results4[m].obj_Id)
                          if (results4[m].obj_Id == addId) {
                            sweetExist = true
                          }
                        }
                        console.log(sweetExist)
                        if (err) {
                          res.json({ code: 500, msg: '判断亲密度表中是否有二人失败' })
                        } else if (sweetExist == false) {
                          console.log('添加')
                          personalDAO.insertsweet(userId, addId, propsweet, function(err, results6) {
                            if (err) {
                              res.json({ code: 500, msg: '添加两人到亲密表失败' + err })
                            } else {
                              res.json({ code: 200, affectedRows: results2.affectedRows + results6.affectedRows, data: propsweet, msg: '送出礼物成功！你与 ' + addId + ' 亲密度增加 ' + propsweet })
                            }
                          })
                        } else if (sweetExist == true) {
                          console.log('修改')
                          personalDAO.updatesweet(userId, addId, propsweet, function(err, results5) {
                            if (err) {
                              res.json({ code: 500, msg: '修改亲密度失败' })
                            } else {
                              res.json({ code: 200, affectedRows: results2.affectedRows + results5.affectedRows, data: propsweet, msg: '送出礼物成功！你与 ' + addId + ' 亲密度增加 ' + propsweet })
                            }
                          })
                        }
                      })
